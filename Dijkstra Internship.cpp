// Dijkstra Internship.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
struct Graph
{
	...
	bool linked = false;    //Dijkistra 遍历判断条件
};
...
void Dijkistra(Graph &graph, int begin, int destination, char *priority)
{
	int start = begin;    //标记搜索开始点
	...
	for (int i = 0; i < graph.vexnum; i++)
	{
		way[i].city.push_back(begin);
		way[i].info.push_back(graph.arcs[begin][i].info);
		way[i].transport.push_back(graph.arcs[begin][i].transport);
		if (strcmp(priority, "TIME") == 0)
			way[i].cost.push_back(graph.arcs[begin][i].time);
		else if (strcmp(priority, "PRICE") == 0)
			way[i].cost.push_back(graph.arcs[begin][i].cost);
	}

	//时间优先
	if (strcmp(priority, "TIME") == 0)     //判断时间优先还是价格优先
	{
...
void VisualDisplay(City *city, Route *route, char *departure, char *destination, const char* address_file_save, Key *key)
{
	...
	fprintf(fp, "</script><title>Shortest path from %s to %s", departure, destination);
...
int main()
{
	...
	const char file_city[19] = "..\\Data\\cities.csv";     //文件地址
	const char file_route[19] = "..\\Data\\routes.csv";
	const char file_save[20] = "..\\Dijkstra_map.htm";
	...
	printf("please enter your desitination city:\n");
	cin.getline(departure, 30);
	printf("please enter your origin place:\n");
	...
	//strcpy(priority,"PRICE");           //TIME OR PRICE
	...
	time1.Stop();
	printf("\nTime for creating graph is: <%4.2lf> ms\n", time1.ElapsedTime());
	...
	time2.Stop();
	printf("\nTime for Dijksitra is: <%4.2lf> ms\n", time2.ElapsedTime());
